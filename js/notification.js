document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadNotifications() {
        try {
            notificationList.innerHTML = '<li class="loading-notice">Loading notifications...</li>';
            
            const response = await fetch('./api/orderapi.php?action=fetchNotifications', {
                headers: {
    ...
    function renderNotifications(notifications) {
        ...
        if (notifications.length === 0) {
            notificationList.innerHTML = '<li class="empty-notice">No notifications yet</li>';
            return;
        }
